...
func TestExpression_ExpressionGroup(t *testing.T) {
	t.Run("can evaluate a scalar", func(t *testing.T) {
		expr := Expression{}
		expr.AddDamageScalar(2, "Damage", tag.NewContainerFromString("Slashing"))
		res := expr.EvaluateGroup()
		assert.Equal(t, 2, res.Value)
		assert.Len(t, res.Terms, 1)
		assert.Equal(t, res.Terms[0].Tags, tag.NewContainerFromString("Slashing"))
	})
	t.Run("groups by tags", func(t *testing.T) {
		expr := Expression{}
		expr.AddDamageScalar(2, "Damage", tag.NewContainerFromString("Slashing"))
		expr.AddDamageScalar(4, "Damage", tag.NewContainerFromString("Slashing"))
		expr.AddDamageScalar(3, "Damage", tag.NewContainerFromString("Magical"))
		res := expr.EvaluateGroup()
		assert.Equal(t, 9, res.Value)
		assert.Len(t, res.Terms, 2)
		assert.Equal(t, res.Terms[0].Tags, tag.NewContainerFromString("Slashing"))
		assert.Equal(t, res.Terms[1].Tags, tag.NewContainerFromString("Magical"))
	})
	...
	t.Run("should halve damage for match tags", func(t *testing.T) {
		...
		expr.AddDamageScalar(2, "Damage", tag.NewContainerFromString("Slashing", "fire"))
