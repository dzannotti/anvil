...
func TestExpression_ExpressionGroup(t *testing.T) {
	t.Run("can evaluate a scalar", func(t *testing.T) {
		...
		expression.AddDamageScalar(2, "Damage", tag.ContainerFromString("Slashing"))
	...
	t.Run("groups by tags", func(t *testing.T) {
		expression := Expression{}
		expression.AddDamageScalar(2, "Damage", tag.ContainerFromString("Slashing"))
		expression.AddDamageScalar(4, "Damage", tag.ContainerFromString("Slashing"))
		expression.AddDamageScalar(3, "Damage", tag.ContainerFromString("Magical"))
		res := expression.EvaluateGroup()
	...
	t.Run("can evaluate a dice", func(t *testing.T) {
		...
		expression.AddDamageDice(1, 6, "Damage", tag.ContainerFromString("Slashing"))
		expression.AddDamageScalar(2, "Damage", tag.ContainerFromString("Slashing"))
		res := expression.EvaluateGroup()
	...
	t.Run("should halve damage for match tags", func(t *testing.T) {
		expression := Expression{rng: &mockRoller{mockReturns: []int{5}}}
		expression.AddDamageDice(1, 6, "Damage", tag.ContainerFromStrings([]string{"Slashing", "fire"}))
		expression.AddDamageScalar(2, "Damage", tag.ContainerFromStrings([]string{"Slashing", "fire"}))
